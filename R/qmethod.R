...
qmethod <- function(dataset,
  ...
  # if (nqsorts != ncol(dataset))
  #   stop("Q method input: The number of Q sorts introduced does not match with the number of columns of the data frame or matrix")
  # else if (nstat != nrow(dataset))
  #   stop("Q method input: The number of statements introduced does not match with the number of rows of the data frame or matrix.")
  # else

  # Validation checks
  if (nstat < 2)
    stop("Q method input: The data frame or matrix entered has less than two statements")
  if (nqsorts < 2)
    stop("Q method input: The data frame or matrix entered has less than two Q-sorts")
  ...
  if (forced) {
    ...
    if (sum(apply(dataset, 2, function(x) sort(x) != qscores)) > 0)
      stop("Q method input: The argument 'forced' is set as 'TRUE', but your data contains one or more Q-sorts that do not to follow the same distribution. For details on how to solve this error, see 'help(qmethod)', including Note.")
  }
  if (!forced) {
    if (is.null(distribution))
      stop("Q method input: The argument 'forced' is set as 'FALSE', but no distribution has been provided in the argument 'distribution'. For details on how to solve this error or specify the argument 'distribution', see 'help(qmethod)', including Note number 2.")
    if (length(distribution) != nrow(dataset))
      stop("Q method input: The length of the distribution provided does not match the number of statements.")
    if (!is.numeric(distribution) & !is.integer(distribution))
      stop("Q method input: The distribution provided contains non-numerical values.")
  }
  if (length(unique(colnames(dataset))) != nqsorts)
    stop("Q method input: one or more Q-sort names are duplicated. Please change the names of the dataset by using colnames().")
  if (rotation != "varimax")
    warning("Note that the rotation method selected is not standard in Q methodology publications.") # See discussion at https://github.com/aiorazabala/qmethod/issues/95
  uncommon.rotations <- c("quartimax", "bentlerT", "geominT", "targetT",
                          "bifactor", "TargetT", "equamax", "varimin",
  ...
  qmethodresults$brief$info        <- c(
    "Q-method analysis.",
    paste0("Finished on:               ", qmethodresults$brief$date),
    paste0("'qmethod' package version: ", qmethodresults$brief$pkg.version),
    paste0("Original data:             ", qmethodresults$brief$nstat,
           " statements, ", qmethodresults$brief$nqsorts, " Q-sorts"),
    paste0("Forced distribution:       ", qmethodresults$brief$distro),
    paste0("Number of factors:         ", qmethodresults$brief$nfactors),
