...
Inspect the 'loa' and 'flagged' tables carefully to see if you missed any flag.")
  ...
  if (forced) {
    ...
    if (sum(apply(dataset, 2, function(x)
      sort(x) != qscores)) > 0)
  ...
  if (!forced) {
    if (is.null(distribution))
      stop("Q method input: The argument 'forced' is set as 'FALSE', but no distribution has been provided in the argument 'distribution'.")
  ...
  brief$info <- c(
    ...
    paste0("Original data:           ", brief$nstat, " statements, ",
  ...
  names(qmethodresults) <- c("brief", "dataset", "loa", "flagged", "zsc",
                             "zsc_n", "f_char")
